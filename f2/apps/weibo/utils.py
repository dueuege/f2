...
# 通用字段名 -> 微博字段名 (Canonical name -> Weibo field)
# 微博没有 author_handle 概念，该字段会被自动跳过。
# (Weibo has no author_handle concept; that field is skipped automatically.)
AUTHOR_FIELD_MAP = {
    "nickname": "nickname",
    "nickname_raw": "nickname_raw",
    "author_id": "uid",
    "work_id": "weibo_id",
    "create_time": "weibo_created_at",
    "desc": "weibo_desc",
}

# 由其他字段拼接而成的字段 (Fields composed from other fields)
AUTHOR_URL_TEMPLATES = {
    "author_url": "https://weibo.com/u/{author_id}",
    "work_url": "https://weibo.com/{author_id}/{work_id}",
}


def format_author_file_name(naming_template: str, weibo_data: dict) -> str:
    """
    ...
    return format_sidecar_name(naming_template, weibo_data, AUTHOR_FIELD_MAP)


def format_author_file_content(fields: list, weibo_data: dict) -> str:
    """
